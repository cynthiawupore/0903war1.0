...
var ChainRxn = new Class({
	...
	doNextLevel: function (score) {
		this.levelNumber++;
		this.score += score;
		this.notifierTitle.set("text", "成功击退日军！累计得分" + this.score);
	...
	newGame: function () {
		...
		this.notifierTitle.set("text", "炸掉" + this.levels[this.levelNumber][0] + "日军");
		this.notifierButton.set('text', "开炸");
		this.notifierButton.removeEvents();
		this.notifierButton.addEvent("click", function () {
...
function shareTitle(score,level){
	...
	if (level < 9) {
		str = "我炸了" + score +"分,停留在第"+ level +"关,击败了全国"+honor +"%的人,日军学界对我尚未定论";
	}
	if (level >= 9 && score > 45000001) {
		str = "我炸了" + score +"分就过关了,击败了全国"+ honor +"%的人,成为了日军皇帝！";
	}
	if (level >= 9 && score < 45000001) {
		str = "我炸了" + score +"分过关,击败了全国"+ honor +"%的人,日军里有毒！";
	}
	return str;
}